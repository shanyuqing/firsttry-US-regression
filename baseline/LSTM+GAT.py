...
class LSTM_GAT_Model(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, lstm_layers=2, num_heads=4):
...
lstm_layers = Lstm_gat_Config.lstm_layers 


model  = LSTM_GAT_Model(input_size, hidden_size, output_size, lstm_layers, num_heads).to(device)
...
plt.legend()
plt.show()
plt.savefig('/root/firsttry/baseline/LSTM+GAT.png')
...
plt.ylabel('value')
plt.legend()
plt.savefig('/root/firsttry/baseline/LSTM+GAT_testing_real_forecast.png')
