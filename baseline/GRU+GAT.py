...
class GRU_GAT_Model(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, gru_layers=2, num_heads=4):
        super(GRU_GAT_Model, self).__init__()
        
        # GRU layer
        self.gru = nn.GRU(input_size, hidden_size, num_layers=gru_layers, batch_first=True)
...
gru_layers=Gru_gat_Config.gru_layers

model = GRU_GAT_Model(input_size, hidden_size, output_size, gru_layers, num_heads).to(device)
...
plt.legend()
plt.show()
plt.savefig('/root/firsttry/baseline/GRU+GAT.png')
...
plt.ylabel('value')
plt.legend()
plt.savefig('/root/firsttry/baseline/GRU+GAT_testing_real_forecast.png')
