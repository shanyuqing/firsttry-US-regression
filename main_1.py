...
# 定义训练模型
def train_model(model, train_data, val_data, epochs, lr, beta):
    print("Training...")
    ...
    for epoch in range(epochs):
        ...
        for i in train_data:
            ...
            y_pred, emb1, com1, emb= model(x, sadj, edge_index)
            optimizer.zero_grad()
            y=y.view(-1,1)
        ...
        # 验证集评估
        val_loss = evaluate_model(model, val_data, beta)
...
# 模型验证
def evaluate_model(model, val_data, beta):
    model.eval()  # 切换到评估模式
    val_loss = 0.0
...
if __name__ == "__main__":
    ...
    # 创建模型
    model = SFGCN(input_dim = time_window, hidden_dim1 = hidden_dim1, hidden_dim2 = hidden_dim2, hidden_dim3=hidden_dim3, hidden_dim4=hidden_dim4, num_layers = num_layers, dropout = dropout).to(device)

    # 训练模型
    loss_values, val_loss_values, IC = train_model(model, train_data, val_data, epochs=epochs, lr=learning_rate, beta=beta)
